...
def main(args):
    ...
    data,features,labels,in_feats,n_classes,train_nid,val_nid,test_nid,device=utils.get_init_info(args,dataset,device)
    ...
    for epoch in range(args.n_epochs):
        model.train()
        acc = utils.evaluate(model, data, test_nid, args.batch_size, device, args.sample_list)
        acc_all.append(acc)
        t0 = time.time()
