...
class CrossEntropyLoss(nn.Module):
    ...
    def forward(self, z, pos_edge_index, neg_edge_index):
        pos_score = self.decoder(z, pos_edge_index, None)
        neg_score = self.decoder(z, neg_edge_index, None)

        score = torch.cat([pos_score, neg_score])
        label = torch.cat([torch.ones(pos_score.size(0)), torch.zeros(neg_score.size(0))]).long()
...
def run(proc_id, n_gpus, args, devices, data):
    ...
    # 提取训练/验证/测试节点的ID
    train_nid = torch.where(train_mask)[0]
    val_nid = torch.where(val_mask)[0]
    test_nid = torch.where(test_mask)[0]
    
    # Create PyTorch DataLoader for constructing blocks
    n_edges = g.edge_index.size(1) // 2
    print(n_edges)
...
def main(args, devices):
    ...
    # g.create_formats_()  it belongs to dgl
